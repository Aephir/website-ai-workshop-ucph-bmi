...
10. **Run the `citation-verifier` skill** against the completed draft and reference list. When the environment permits delegation and the user has authorized sub-agent work, delegate it to a fresh, read-only sub-agent so the reviewer does not inherit the drafting rationale. Otherwise run it in the current session and disclose that the pass was not context-independent. It reports findings for a human (or this skill, on the human's instruction) to act on rather than silently patching the draft.

11. **Run `ai-slop-check`** against the completed draft before calling it done, especially its redundancy and rhythm passes. Optionally run `scripts/phrase_sweep.py` for a fast deterministic banned-phrase check. The script cannot detect cross-section repetition or monotonous prose rhythm; those require the skill's full-document review.
